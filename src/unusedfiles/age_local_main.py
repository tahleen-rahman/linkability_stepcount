...
for hr in [0.05, 0.1, 0.2, 0.25, 0.5, 1, 2, 3, 4, 6, 8, 12, 24]:
    ...
    for st in stats[1:]:  # beacuse 1st element is the null set  #( ['max'],['max', 'std'],['max', 'std', 'medi'], ['max', 'std', 'medi', 'mean']):
        ...
        inf = AttributeInference(ds.out_name, 'age_binary', in_datapath=stats_datapath)
...
best results from statistics
1. age_binary- stats_dzne_week_max_medi 3h,  0.7776942355889725
2. sex- stats_dzne_week_max 6h, 0.653409090909091
3. edu_binary- stats_dzne_week_sum_max_medi_mean 2h, 0.6571428571428571

In general age_binary has the best results with statistics. 
