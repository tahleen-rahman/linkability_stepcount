...
epochs, regu, batchsize = int(sys.argv[1]), float(sys.argv[2]),  int(sys.argv[3]) #150,0.001,64,

weekend = True
aucfname = str(weekend) + "_"+ str(epochs) + "_" + str(regu) + "_" + str(batchsize) + "_aucs_med.csv"
arr, aucarr = [], []

#epochs, combi, regu = int(sys.argv[1]), int(sys.argv[2]), sys.argv[3]

for infile in ['dzne_dsp_dist_4_720_nor', 'dzne_dsp_dist_4_720']: #os.listdir("../data/dzne/linkdata_med/"): # all  intervals for single stats + distributions 1, 6, 12 hrs
