...
class AttributeInference(Attack):
    ...
    def makeDataset(self):
        ...
        if self.attribute=='age_binary':
            ...
            young_u_tr = young_users[:int(0.8 * len(young_users))]

            young_train = youngs[youngs.user.isin(young_u_tr)]
            young_test = youngs.drop(young_train.index)

            olds = df[df.age_binary == 1]
            old_users = olds.user.unique()
            old_u_tr = old_users[:int(0.8*len(old_users))]

            old_train = olds[olds.user.isin(old_u_tr)]
            old_test = olds.drop(old_train.index)
    ...
    def attack_activities(self, clf,  ths = [0.5]):
        """
        ...
        for th in ths:
            ...
            results.append((th, auc_vote_bin, auc_proba_1_bin))
